...
var (
	connectionString = testutil.BigCacheConnectionString
...
func TestBigCache_NewCache_WithNilURL(t *testing.T) {
	_, err := newCache(nil)
	if err == nil {
...
func TestBigCache_NewCache_WithInvalidURL(t *testing.T) {
	testCases := []struct {
		conn string
	}{
		{"bigcache:"},
		{"bigcache://"},
		{"bigcache://?eviction=5z"},
	}
